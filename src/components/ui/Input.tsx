...
const Input = forwardRef<HTMLInputElement, InputProps>(
  ({ label, error, hint, className, id, type, ...props }, ref) => {
    ...
    // Campos de senha ganham um botão de "ver senha" (olhinho).
        ...
        <div className="relative">
          ...
          {isPassword && (
            <button
              ...
              className="absolute inset-y-0 right-0 flex items-center px-3 text-brand-gray-400 hover:text-brand-black transition-colors"
              tabIndex={-1}
            >
              {show ? <EyeOff /> : <Eye />}
            </button>
